...
def load_whatsapp_data(filename):
    #text = smart_str(open(filename).read()).decode('utf-8', 'ignore')
    text = smart_str(open(filename).read())
    #print text
...
def main(filename):
    ...
    messages = load_whatsapp_data(filename=filename_message)
    print('\t>>> Total Text Messages: ', len(messages))
    ...
    insert_whatsapp_messages_on_database(conn=conn, messages=messages)
...
def list_all_files():
    ...
    if len(sys.argv) >=3:
        ...
        for date in dates:
            ...
            filename = files_path+'all_text_'+date+'.json'
            main(filename)
